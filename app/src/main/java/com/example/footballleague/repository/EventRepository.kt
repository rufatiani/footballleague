...
class EventRepositoryImpl(private val apiInterface: ApiInterface, val context: Context) : EventRepository {
    ...
    override fun saveFavoriteEvent(event: Event) : Result<Long> {
        return try {
            val result = context.database.use {
                insert(Const.TB_FAVORITE_EVENTS,
    ...
    override suspend fun getPrevFavoriteEvents(): Result<List<Event>> {
        return try {
            var result = context.database.use {
            ...
            Result.Success(result.parseList(classParser<Event>()))
        } catch (e: Exception) {
            Result.Error(e)
        }
    }
